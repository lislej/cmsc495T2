...
//	private final String CALC_SERVICE_URL = "http://localhost:8080/LACServiceFinal/services/LoanCalcService";
	...
	private void createAmortizationSchedule() {
		...
		for (int i = 0; i < term; i++) {
			...
			monthlyIntPaid = round(calcMonthlyInterest(balance, rate), 3);
			totalInt += monthlyIntPaid;
			principal = round(calculatePrincipal(payment, monthlyIntPaid),3);
			...
			amortScheduleBody.append(String.format(" %4d    %7.2f     %7.2f     %7.2f     %9.2f", i + 1, payment,
					principal, monthlyIntPaid, balance > 0 ? balance : 0.00));
			amortScheduleBody.append("\n");
	...
	public double round(double data, int decplace)	{		
		return Math.floor(data * Math.pow(10, decplace - 1) + 0.5) / Math.pow(10, decplace - 1);
	}
	...
	private double calcMonthlyInterest(double balance, double intRate) {
		double monthlyIntRate = intRate / (12 * 100);
		return balance * monthlyIntRate ;
	}
	
	private double calculatePrincipal(double loanPmt, double monthlyInterestPaid) {
		return loanPmt - monthlyInterestPaid;
	}
